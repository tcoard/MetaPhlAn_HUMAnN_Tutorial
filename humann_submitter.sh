...
INPUT="${containerdir}/trimmed/evol1.R1.fastq"

### !!! CHANGE !!! make your own output directory
OUTPUT_DIR=${containerdir}/CHANGE_THIS/
PROJECT=Tutorial11

singularity exec --bind /ifs/groups/eces450650Grp/ECES450650_SP21/Tutorial11/:/${containerdir},${TMP}:/tmp,${TMP}:${TMP} docker://biobakery/humann humann --input ${INPUT} --output ${OUTPUT_DIR} --nucleotide-database ${containerdir}/databases/chocophlan/ --protein-database ${containerdir}/databases/uniref --metaphlan-options='--bowtie2db /tmp/bowtie/' --threads 48
